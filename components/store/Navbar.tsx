    ...
    <nav className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-neutral-100">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex items-center justify-between h-16">
          {/* Logo / shop name */}
          <Link href={base} className="text-xl font-bold tracking-tight text-neutral-900">
            {branding.logoUrl ? (
          ...
          <div className="flex items-center gap-5">
            ...
            <button className="hidden sm:block text-neutral-500 hover:text-neutral-900 transition-colors">
              <User className="w-[18px] h-[18px]" strokeWidth={1.5} />
            </button>
            <Link href={`${base}/cart`} className="text-neutral-500 hover:text-neutral-900 transition-colors relative">
          ...
          <ul className="px-6 py-4 space-y-3">
            {NAV_ITEMS.map((item) => (
              <li key={item.label}>
                <Link
                  href={item.href}
                  className="block text-sm tracking-wide text-neutral-600 hover:text-neutral-900 py-1.5"
